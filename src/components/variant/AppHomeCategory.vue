    ...
    <v-menu :open-on-hover="false" close-on-back transition="fade-transition" open-delay="0" close-delay="0"
        ...
        <template #activator="menu">
            <v-list-item class="text-body-2" base-color="primary" height="50" variant="flat" rounded link v-bind="menu.props"
                ...
                <router-link :to="`/products?category=${category.id}`" class="text-white text-decoration-none">{{ category[`name_${locale as 'uz'}`] }}</router-link>
            </v-list-item>
        </template>
        <v-list density="compact" elevation="1" max-height="400">
            <v-list-item link v-for="ct1, j in category.children" :key="j">
                <router-link :to="`/products?category=${ct1.id}`" class="text-black text-decoration-none">{{ ct1[`name_${locale as 'uz'}`] }}</router-link>
                <template #append>
                    <v-menu :close-on-content-click="true" location="right" offset="30" transition="fade-transition">
                        ...
                        <v-list elevation="1" density="compact">
                            <v-list-item link v-for="ct2, o in ct1.children" :key="o">
                                <router-link :to="`/products?category=${ct2.id}`" class="text-black text-decoration-none">{{ ct2[`name_${locale as 'uz'}`] }}</router-link>
                            </v-list-item>
                        </v-list>
                    </v-menu>
                </template>
