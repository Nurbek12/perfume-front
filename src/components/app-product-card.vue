<template>
  <v-card width="100%" flat class="text-center">
    ...
    <v-card-text class="py-0 text-overflow-3">{{ product[`description_${locale as "uz"|"ru"|"en"}`] }}</v-card-text>
    ...
    <v-card-actions class="d-flex justify-space-between">
        <div class="w-100 mr-2">
            <div v-if="!!cart_item(product.id!)" class="w-100 d-flex justify-space-between align-center">
                ...
                <v-btn  :disabled="product.units[0].balance <= cart_item(product.id!).quantity" size="40" color="primary" variant="flat" class="text-none ml-0" @click="commit('ADD_TO_CART', product)"><v-icon>mdi-plus</v-icon></v-btn>
            </div>
            <v-btn :disabled="product.units[0].balance == 0" height="40" block v-else @click="commit('ADD_TO_CART', product)" color="primary" variant="flat" class="text-none">Добавить в корзину</v-btn>
        </div>
