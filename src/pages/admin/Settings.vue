    ...
    <v-row>
      <v-col cols="12" sm="4">
        <v-card class="mx-auto">
          <v-card-title class="font-weight-light">Настройки Авы</v-card-title>
          ...
          <v-card-subtitle class="font-weight-light">Настройки пароля</v-card-subtitle>
          <v-card-text>
            <v-row>
              <v-col cols="12">
                <v-text-field
          ...
          <v-card-actions>
            <v-row class="px-2 pb-2">
              <v-col cols="12">
                <v-btn color="primary" height="42" @click="changePass" variant="elevated" block>Изменить Пароля</v-btn>
              </v-col>
            </v-row>
          </v-card-actions>
        </v-card>
      ...
      <v-col cols="12" sm="8">
        <v-card>
          <v-card-title class="font-weight-light">Настройки аккаунта</v-card-title>
...
const details: Ref<IUser> = ref({...getters.user} as IUser)
const pass1 = ref('')
const pass2 = ref('')
...
const imagesrc = computed(() => {
  ...
  else if(details.value.image) return details.value.image
...
const changePass = async () => {
  ...
  if(!data) return
  commit('SET_USER', {...getters.user, password: pass1.value})
