        ...
        <v-card-text class="px-4 pt-2 pb-3">
          <v-form class="w-100 form-card" ref="form">
            <v-row class="pa-2">
              ...
              <v-col cols="12" class="pa-2">
                  <v-textarea :rules="nameRule" no-resize flat class="border rounded" density="compact" bg-color="surface" v-model="editedItem.description_uz" placeholder="Ma'lumot (uz)" hide-details color="primary" variant="solo"></v-textarea>
              </v-col>
              <v-col cols="12" class="pa-2">
                  <v-textarea :rules="nameRule" no-resize flat class="border rounded" density="compact" bg-color="surface" v-model="editedItem.description_ru" placeholder="Описание (ru)" hide-details color="primary" variant="solo"></v-textarea>
              </v-col>
              <v-col cols="12" class="pa-2">
                  <v-textarea :rules="nameRule" no-resize flat class="border rounded" density="compact" bg-color="surface" v-model="editedItem.description_en" placeholder="Description (en)" hide-details color="primary" variant="solo"></v-textarea>
              </v-col>
              <v-col cols="12" class="pa-2">
                  <v-row>
...
import { IBrand, IBrandForm } from '../../interfaces/index.variant'
import { createBrand, deleteBrand, getAllBrands, updateBrand } from '../../api/brands'
...
const editedItem = ref<IBrandForm|any>({
  name: '',
  country: null,
  description_uz: "",
  description_ru: "",
...
const save = async () => {
  ...
  if (editedIndex.value > -1) {
    const { data } = await updateBrand(editedId.value as any, form_data)
